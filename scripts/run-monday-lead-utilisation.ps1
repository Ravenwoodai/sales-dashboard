...
$arguments = @(
  "run", "report:weekly-lead-utilisation", "--",
  "--config", $configPath,
  "--start", $StartDate
)
...
Write-Host "Weekly Lead Utilisation: $StartDate to $friday" -ForegroundColor Cyan
...
if ($DryRun) {
  ...
  Write-Host "Complete. Reports are under: $projectRoot\outputs\weekly-lead-utilisation\$runId" -ForegroundColor Green
}
