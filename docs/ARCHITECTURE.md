...
13. `src/evaluationStudio.js` defines the governed Evaluation Studio foundation: the neutral three-lens Call Intelligence Foundation, deterministic specialist routing, seeded Neuron/LatentPulse-derived knowledgebase entries, editable strict-schema templates with safe custom evaluation goals, a manager-calibrated three-outcome Offer Acceptance evaluator with semantic validation, queued/prompt-test run metadata, run snapshots, run quarantine/resume history, all-eligible-call batch selection, versioned result records with prompt and knowledgebase-version provenance, filter-aware evidence/result queues, Foundation and specialist rollups, result-to-manager-review handoff helpers, safe suggested correction prefill, and local model guardrails.
...
- The seeded Offer Acceptance template is added non-destructively to existing stores. Its fixed schema keeps category/classification pairs consistent, verifies quoted offer/customer evidence against the transcript, blocks category 3 when an approval or decision remains unresolved, and stores a separate acceptance assessment plus normalized Studio findings. Category 3 is an accepted-offer transcript signal, not a completed commercial outcome.
