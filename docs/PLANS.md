...
- Scope shipped: seeded active `Offer Acceptance (Sale Signal) Review` template, strict `offer_acceptance_classification.v1` result schema with a numeric integer category contract, non-destructive current-store reconciliation, transcript-quote verification, category/classification consistency checks, category 3 commitment and unresolved-condition enforcement, stored acceptance assessment, and visible normalized Studio findings. Active template v3 requires short contiguous evidence excerpts, complete chronological review, and final-customer-position precedence; local ingestion may shorten a quote only when it remains verified verbatim and independently downgrades an earlier acceptance when a later customer turn withdraws it or adds an unresolved condition. Studio shows accepted/follow-up/no-signal outcomes, acceptance rate, latest-run failures, salesperson/source/date breakdowns, and the evaluation template/run type/version used for every result instead of generic lead-audit columns. Recoverable historical quote-format failures are reprocessed once per validation revision, and recovery defers when the source transcript is unavailable.
...
### Auditable Call Commercial Lifecycle
- Goal: make a call's acceptance and downstream commercial state precise, useful, and safe for management reporting without turning transcript acceptance into paid revenue.
- Scope shipped: versioned call aggregate and commercial-state schemas; authoritative Offer Acceptance conflict resolution; typed Callback/Procedure/Objection specialist contracts; exact field-relevant proof; strong historical transcript-evidence selection; source-date relative-date resolution; per-specialist lifecycle states; claim provenance; uncalibrated-confidence audit metadata; and call/result UI/API presentation.
- Follow-up semantics: a later exact stable-ID call is `later_attempt_observed`, with match basis and later call context retained. Completion and payment stay `not_established` unless separately verified.
- Compatibility: retired seeded specialist v1 templates are archived non-destructively, historical generic results remain readable, and user-created safe custom templates are preserved. Report rollups count each signal once per evaluation result.
