...
- `src/callIntelligenceAggregate.js`: read-only call-level authority, commercial lifecycle, specialist state, conflict, provenance, exact-evidence, and relative-date projection shared by result APIs and call/result UI.
...
- The call commercial-state projection keeps offer acceptance, quoted value, intended payment timing, payment verification, invoice, fulfilment, revenue, and CRM state independent. Relative dates use the source call's Australian DD/MM/YYYY date and retain the raw phrase plus ambiguity policy. Later stable-ID calls are related attempts, never automatic proof of completion.
...
- Default specialist templates for Callback Opportunity, Procedure Adherence, and Objection Handling use strict typed v2 contracts with exact evidence and local normalized findings. Historical generic results and user-created safe custom templates remain readable and active according to their own contracts.
