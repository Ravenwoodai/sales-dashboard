...
- Provide a standalone Evaluation Studio for the local workflow: choose transcripts by call IDs or operational filters, preview the exact batch, run a selected strict-schema evaluator, and browse paginated results by business outcome, evidence, confidence, specialist type, and operational issue. This includes the neutral Call Intelligence Foundation, which records opportunity, measurement eligibility, and efficiency as three independent lenses and can selectively route calls to the five existing specialist evaluators, plus the manager-calibrated Offer Acceptance evaluator with exactly three operational outcomes: no sale signal, interested/follow-up only, and customer accepted the offer. Knowledgebase entries are local drafts or included references; no manager identity or approval is required. Optional calibration uses immutable local reference labels and remains separate from ordinary evaluation. Results never overwrite raw calls or operational records.
